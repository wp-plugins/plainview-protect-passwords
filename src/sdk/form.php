...
/**
	@brief	plainview\sdk_propas\form is a XHTML form class that handles creation, display and validation of form elements and complete form layouts.
	@par	Using the class
...
class form
{
	...
	{
		$this->options = \plainview\sdk_propas\base::merge_objects( $this->default_options, $options );
	...
	**/
	...
	{
		$options = \plainview\sdk_propas\base::merge_objects( $this->options, $options );
	...
	public function make_input( $options )
	{
		...
		switch ( $options->type)
		{
			...
			case 'radio':
				// Make the options
				$rv = '';
				$baseOptions = \plainview\sdk_propas\base::merge_objects( $this->options, $options);
			...
			switch( $input->type )
			{
				case 'text':
					...
					if (isset( $input->email) )
					{
						...
						if ( ! \plainview\sdk_propas\wordpress\base::is_email($email) )
